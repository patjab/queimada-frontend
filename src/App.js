...
class App extends Component {
  ...
  setSocket = () => {
    this.setState({
      socket: this.state.socket || io("http://10.39.111.234:8080")
    }, () => {
      ...
      <Fragment>
        ...
        <Switch>
          ...
          <Route path='/signup' render={() => {
            return this.state.currentUser ? <QueimadaContainer socket={this.state.socket} allUsers={this.state.allUsers} currentUser={this.state.currentUser} friendRequests={this.state.friendRequests} createdFriendRequest={this.state.createdFriendRequest}/> : <AuthAction submitAuthAction={this.signUp} authType='signup' errors={this.state.errors}/>
          }}/>
